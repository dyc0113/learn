		...
		for(int i = 0; i < epoll_events_count ; i++)
		{
			...
			if(events[i].data.fd == listener)
			{
			   ...
			   /*bzero(message, BUF_SIZE);
                ...
                cout<<"开始处理数据---------------------------------------"<<endl;
                int iRet = handle_file_message(events[i].data.fd);
                if (iRet != 0)
                {
...
int handle_file_message(int client)
{
    ...
    iRet =saf_tcp_recv(client, g_buf, sizeof(SCommandReq));
    if (iRet != 0)
    {
    ...
    if (pReq->IsHaveData())
    {
         cout<<"recv content len:"<<pReq->len - sizeof(SCommandReq)<<endl;
         iRet =saf_tcp_recv(client, g_data, pReq->len - sizeof(SCommandReq));
         if (iRet != 0)
         {
            cerr<<"reve data failed"<<endl;
            return -1;
         }
         cout<<"content:"<<g_data<<endl;
    }
    SCommandRsp stRsp;
    ...
    do
    {
		...
		cout<<"filepaht:"<<filePath.c_str()<<endl;
        if( access(filePath.c_str(), 0) == 0)// 文件存在
        {
            ...
            if (pReq->create_tm >= (uint64_t)stStat.st_mtim.tv_sec)// 修改时间较早，覆盖
            {
                if (pReq->IsHaveData()) // 如果有数据
                {
                    ...
                    iRet =  writeFile(filePath, g_data, pReq->len - sizeof(SCommandReq));
                    if (0 != iRet)
                    {
        ...
        else // 文件不存在
        {
            ...
            if (pReq->IsHaveData() || pReq->fileName[0]!='\0') // 如果有数据,或者有文件名
            {
                // 进行数据写入
                // 从socket当中读入数据
                iRet =  writeFile(filePath, g_data, strlen(g_data));
                if (0 != iRet)
                {
                    cerr<<"write file failed"<<endl;
                    stRsp.ret = CONSTANT_WRITE_FILE_FAILED;
    ...
    // 组装会送报文
    cout<<"相应报文长度:"<<stRsp.len<<endl;
    iRet = safe_tcp_send(client, (void*)&stRsp, stRsp.len);
    if (0 != iRet)
    {
