...
const Int Value::minInt = INT_MIN;
const Int Value::maxInt = INT_MAX;
const UInt Value::maxUInt = UINT_MAX;
const Int64 Value::minInt64 = LLONG_MIN;
const Int64 Value::maxInt64 = LLONG_MAX;
const UInt64 Value::maxUInt64 = ULLONG_MAX;
...
Value::CommentInfo::setComment( const char *text )
{
   if ( comment_ )
      valueAllocator()->releaseStringValue( comment_ );
   //JSON_ASSERT( text );
   JSON_ASSERT_MESSAGE( text && (text[0]=='\0' || text[0]=='/'), "Comments must start with /");
...
Value::operator[]( UInt index )
{
   //JSON_ASSERT( type_ == nullValue  ||  type_ == arrayValue );
   JSON_ASSERT_MESSAGE(type_ == nullValue || type_ == arrayValue,
                       "in Json::Value::operator[](UInt): requires arrayValue");
   if ( type_ == nullValue )
      *this = Value( arrayValue );
#ifndef JSON_VALUE_USE_INTERNAL_MAP
   CZString key( index );
