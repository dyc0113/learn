		...
		case 0: // child process
			{
				...
				SCommandReq *pReq = (SCommandReq*)new char[sizeof(SCommandReq) + 100]; 

				cout<<"sizeof(ScommandReq)"<<sizeof(SCommandReq)<<endl;
				time_t now = time(NULL);
				pReq->create_tm = now;
				strcpy(pReq->dir, "/home/dyc0113/git/synFileServer");
				strcpy(pReq->fileName, "1.txt");
			    strcpy(pReq->data, "1234567890");
				pReq->len = sizeof(SCommandReq) + 100;
				memcpy(message, (char*)pReq, sizeof(SCommandReq)+100);
				...
				if(strncasecmp(message, CMD_EXIT, strlen(CMD_EXIT)) == 0)
				{
					...
					CHK(write(pipe_fd[1], message, pReq->len));
			...
			while(continue_to_work) {
				...
				for(int i = 0; i < epoll_events_count ; i++){
					...
					if(events[i].data.fd == sock){
						...
						SCommandReq *pReq = (SCommandReq*)message;
						cout<<"len:"<<pReq->len<<endl;
						cout<<"fileName:"<<pReq->fileName<<endl;
						// zero size of result means the child process going to exit
						if(res == 0) continue_to_work = 0; // exit parent to
						// send message to server
						else{
							CHK(send(sock, message, pReq->len, 0));
						}
					}
				}
			}
